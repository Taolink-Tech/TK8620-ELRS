...
void luadevGeneratePowerOpts(luaItem_selection_t *luaPower)
{
  ...
  while (pwr < POWERMGNT_getMinPower())
  {
    while (*out++ != ';') ;
  ...
  // Continue until after the max power item and terminate the option string
  // at the separating semicolon.
  while (pwr <= POWERMGNT_getMaxPower())
  {
