...
typedef struct crsf_header_s
{
    ...
    uint8_t type; // crsf_frame_type_e
    uint8_t payload[0];
...
typedef struct crsf_ext_header_s
{
    ...
    uint8_t type; // crsf_frame_type_e
    // Extended fields
    uint8_t dest_addr; // crsf_addr_e
    uint8_t orig_addr; // crsf_addr_e
    uint8_t payload[0];
...
typedef struct crsf_sensor_gps_s
{
    ...
    uint16_t altitude; // meters with a -1000 m offset
...
typedef struct elrsLinkStatistics_s
{
    crsfLinkStatistics_t crsfLinkStatistics;
