...
static void GenerateChannelData8ch12ch(OTA_Packet8_s * const ota8, const uint32_t *channelData, bool const TelemetryStatus, bool const isHighAux)
{
    ...
    // ota8->rc.uplinkPower = constrain(CRSF_GetLinkStatistics()->crsfLinkStatistics.uplink_TX_Power, 1, 8) - 1;
...
static void UnpackChannelDataHybridCommon(OTA_Packet4_s const * const ota4, uint32_t *channelData)
{
#if defined(DEBUG_RCVR_LINKSTATS)
 ...
 */
static bool UnpackChannelDataHybridSwitch8(OTA_Packet_s const * const otaPktPtr, uint32_t *channelData,
 ...
 * Returns: TelemetryStatus bit
 */
static bool UnpackChannelDataHybridWide(OTA_Packet_s const * const otaPktPtr, uint32_t *channelData, uint8_t const tlmDenom)
{
    ...
    if (switchIndex == 7)
    {
        CRSF_updateUplinkPower(switchByte & 0b111111);
    }
    else
    {
        uint8_t bins;
...
bool UnpackChannelData8ch(OTA_Packet_s const * const otaPktPtr, uint32_t *channelData, uint8_t const tlmDenom)
{
    (void)tlmDenom;
    ...
    CRSF_updateUplinkPower((uint8_t)(ota8->rc.uplinkPower + 1));
