...
static uint8_t sendCRSFparam(uint8_t fieldChunk, luaPropertiesCommon_t *luaData)
{
  ...
  uint8_t chunkMax;
  chunkMax = CRSF_MAX_PACKET_LEN - 6 - 2;
  ...
  CRSF_SetExtendedHeaderAndCrc(packetBuf, CRSF_FRAMETYPE_PARAMETER_SETTINGS_ENTRY,
...
bool luaParamUpdatePending(void)
{
  return UpdateParamReq;
}

static void sendLuaDevicePacket(void)
{
...
bool luaHandleUpdateParameter(void)
{
  ...
  switch (parameterType)
  {
    ...
    case CRSF_FRAMETYPE_PARAMETER_WRITE:
    {
      ...
      if (id < LUA_MAX_PARAMS && p && paramCallbacks[id])
      {
