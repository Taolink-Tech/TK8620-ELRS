#!/usr/bin/env python3
r"""TK8620 BootROM flasher.

This follows the TK8620 BurnTool BootROM protocol and uses the public
bootpatch image shipped in this repository.
"""
...
class Tk8620Bootrom:
    ...
    def bootrom_handshake_once(self, timeout_s: float) -> bytes:
        deadline = time.monotonic() + timeout_s
        buf = bytearray()
        sent_taolink = False
        last_turmass_at = 0.0
    ...
    def handshake(self, reset: bool, module_label: str = "module") -> None:
        log("Handshake at 115200...")
        ...
        for attempt in range(1, 4):
            log(f"[{attempt}/3] Press the {module_label} module reset key to enter BootROM.")
            log("Waiting for TurMass.")
            try:
                data = self.bootrom_handshake_once(30.0)
                if b"ok" in data:
                    self.ser.rts = True
                    return
            except TimeoutError as exc:
                last_error = exc
                log(f"BootROM handshake not complete: {exc}")
                if attempt < 3:
                    self.ser.reset_input_buffer()
                    log(f"Check {module_label} module power, then press reset again.")

        raise TimeoutError(f"BootROM handshake failed after reset prompts: {last_error}")

    def get_version(self) -> None:
        log("Get BootROM version...")
        self.command(
            OPCODE_VERSION_GET,
            0,
            0,
    ...
    def download_patch(self, patch: bytes) -> None:
        ...
        for index, offset in enumerate(offsets, 1):
            ...
            if not VERBOSE:
                progress("  patch", index, len(offsets))
    ...
    def change_baudrate(self) -> None:
        if self.target_baud == 115200:
            return
        log(f"Switch baudrate to {self.target_baud}...")
        self.command(
    ...
    def erase_for_local_burn(self, erase_user_params: bool) -> None:
        log("Erase flash blocks 0x00000..0x3FFFF...")
        ...
        if not erase_user_params:
            log("Keep ELRS user parameters 0x69000..0x7EFFF.")
            return

        log("Erase ELRS user parameters 0x69000..0x7EFFF, keep Rx OTA metadata 0x7D000...")
        erase_addresses = [
            address
    ...
    def erase_sectors(self, start: int, length: int, label: str) -> None:
        ...
        log(f"Erase {label} 0x{start:05X}..0x{last - 1:05X}...")
        for index, address in enumerate(erase_addresses, 1):
            debug(f"  erase sector 0x{address:05X}")
            self.command(
                OPCODE_ERASE,
    ...
    def write_local_image(self, image: bytes, label: str) -> None:
        log(f"Write {label} image 0x00000..0x2FFFF...")
    ...
    def write_flash(self, address: int, data: bytes, label: str) -> None:
        ...
        log(f"Write {label} 0x{address:05X}..0x{end - 1:05X}...")
    ...
    def read_flash(self, address: int, length: int, label: str) -> bytes:
        ...
        log(f"Read back {label} 0x{address:05X}..0x{end - 1:05X}...")
        data = bytearray()
    ...
    def verify_crc32_range(self, flash_offset: int, length: int, expected: int, label: str) -> None:
        ...
        log(f"  target=0x{actual:08X} local=0x{expected:08X}")
        if actual != expected:
...
def run_local(args: argparse.Namespace, kind: str) -> int:
    ...
    log(f"Bootloader size: 0x{len(parse_strhex(bootloader_path)):X}")
    log(f"{label} firmware size: 0x{len(parse_strhex(firmware_path)):X}")
    log(f"{label} image size: 0x{len(image):X}")
    log(f"{label} image CRC32: 0x{crc:08X}")
    debug(f"Patch size: 0x{len(patch):X}")
    log(f"ELRS user parameters: {'keep' if args.keep_user_params else 'erase'}")
    ...
    try:
        ...
        log("then unplug/replug the CH340 adapter and rerun burn.cmd.")
        return 2

    try:
        dev.handshake(reset=not args.no_reset, module_label=label)
        dev.get_version()
...
def run_rx_stash(args: argparse.Namespace) -> int:
    ...
    log(f"Rx firmware size: 0x{len(firmware):X}")
    log(f"Rx firmware CRC32: 0x{firmware_crc:08X}")
    log(f"Rx firmware version: {rx_version[0]}.{rx_version[1]}.{rx_version[2]} ({rx_version_source})")
    log(f"Rx OTA frequency: {args.freq}")
    log(f"Rx OTA target ID: {target_id.hex().upper()}")
    log(f"Rx stage image: 0x{args.image_offset:05X}..0x{args.image_offset + len(firmware) - 1:05X}")
    log(f"Rx OTA metadata: 0x{RX_OTA_META_OFFSET:05X}..0x{RX_OTA_META_OFFSET + len(meta) - 1:05X}")
    debug(f"Patch size: 0x{len(patch):X}")
    ...
    try:
        dev.handshake(reset=not args.no_reset, module_label="TX")
        dev.get_version()
        dev.download_patch(patch)
        dev.execute_patch("Execute RAM patch...")
        dev.change_baudrate()
        ...
        log("Rx OTA metadata verify OK.")
        dev.execute_patch("Execute RAM patch for flash lock...")
        dev.disconnect()
        log("Rx firmware stash complete.")
        return 0
    except TimeoutError as exc:
        log(f"Burn failed: {exc}")
...
def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(description="TK8620 BootROM burn tool")
    ...
    def add_local_burn_parser(name: str, label: str) -> None:
        ...
        local.add_argument("--no-reset", action="store_true", help="do not send AT+RST before handshake")
        local.add_argument(
            "--keep-user-params",
            action="store_true",
    ...
    def add_rx_stash_parser(name: str, help_text: str) -> None:
        ...
        rx_stash.add_argument("--no-reset", action="store_true", help="do not send AT+RST before handshake")
